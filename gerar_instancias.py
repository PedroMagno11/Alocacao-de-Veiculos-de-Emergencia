...
import PARAMETROS
from read_instance import calcular_distancia_haversine, obter_nome_coluna_cobertura
...
def gerar_instancia(
    ...
    dataframe = gerar_pontos(
        ...
        quantidade_clusters=argumentos.clusters,
        desvio_cluster_km=argumentos.desvio_cluster_km,
        gerador=gerador,
    )
...
def parse_args():
    ...
    parser.add_argument("--desvio-cluster-km", type=float, default=2.0)
...
def main():
    ...
    for indice, quantidade_pontos in enumerate(argumentos.quantidades, start=1):
        dataframe = gerar_instancia(
            quantidade_pontos=quantidade_pontos,
            indice_instancia=indice,
            argumentos=argumentos,
            ranges=ranges,
        )
        caminho_saida = pasta_saida / (
            f"{argumentos.prefixo}_{indice:02d}_{quantidade_pontos}p.csv"
        )
